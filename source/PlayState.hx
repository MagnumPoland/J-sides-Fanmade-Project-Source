...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['lmao how', 0.2], //From 0% to 19%
		['XD', 0.4], //From 20% to 39%
		['you play like shit', 0.5], //From 40% to 49%
		['hmmmm interesting', 0.6], //From 50% to 59%
		['nerd lmao', 0.69], //From 60% to 68%
		['troll face', 0.7], //69%
		['wowowowowow', 0.8], //From 70% to 79%
		['very cool', 0.9], //From 80% to 89%
		['my gawd', 1], //From 90% to 99%
		['SillyFangirl Osu reference', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "You just suck, don't you?", 32);
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			...
			case 'Dadbattle Spotlight':
				...
				dadbattleLight = new BGSprite('spotlight', 400, -200);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence(detailsText, "SKILL ISSUE ON: " + SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
